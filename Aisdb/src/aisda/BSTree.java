...
public class BSTree {
    ...
    private TreeNode root;    // pierwszy wÄ™zeĹ‚ drzewa
    ...
    public void insert(int elem) {
        TreeNode newNode = new TreeNode();    // tworzymy nowy wÄ™zeĹ‚
        ...
        if (root == null) {
            ...
            while (true) { // (zakoĹ„czenie pÄ™tli wewnÄ…trz za pomoca return)
                TreeNode parent = current;
                if (elem < current.iData) { // czy idziemy w lewo?
                    current = current.left;
                    if (current == null) { // jeĹ›li nie ma lewego,
    ...
    public boolean find(int elem) { //sprawdzenie czy w zbiorze jest element: elem
        if (root == null) {
            return false;       //puste drzewo
        }
        TreeNode current = root;               // zaczynamy od korzenia drzewa
        while (current.iData != elem) { // dopĂłki nie odnaleziono,
            if (elem < current.iData) {// czy przechodzimy na lewo?
                current = current.left;
            } else { // na prawo
                current = current.right;
            }
            if (current == null) { // jeĹ›li brak potomka,
    ...
    private void deleteWithoutChilds(TreeNode current) { //Usuniecie wezla bez potomkow
        if (current == root) { // jeĹ›li to korzeĹ„,
            root = null;                 // oprĂłĹĽniamy drzewo
        } else {
            if (current.parent.left == current) { //usuwany byl lewym synem ojca
                current.parent.left = null;    // odĹ‚Ä…czamy usuwany wÄ™zeĹ‚
            } else {//byl prawym synem ojca
                current.parent.right = null;  // odĹ‚Ä…czamy usuwany wÄ™zeĹ‚
            }
        }
    }
    ...
    private void deleteWithOneLeftChild(TreeNode current) { //Usuniecie wezla z jednym lewym potomkiem
        if (current == root) {
            root = current.left;
        } else if (current.parent.left == current) { //usuwany byl lewym synem ojca
    ...
    public boolean delete(int elem) {// usuniÄ™cie wÄ™zĹ‚a o podanej wartoĹ›ci elementu
        if (root == null) {
            return false;       //puste drzewo
        }
        TreeNode current = root;
        while (current.iData != elem) { // szukamy wÄ™zĹ‚a
            ...
            if (current == null) { // koniec szukania i ...
                return false;                // ... nie znaleziono wÄ™zĹ‚a
            }
        }
        // znaleziono wÄ™zeĹ‚ do usuniÄ™cia
        if (current.left == null && current.right == null) { // usuniecie w przypadku braku potomkow
            deleteWithoutChilds(current);
        } else if (current.right == null) { //jeĹ›li brak prawego potomka, zastÄ™pujemy wÄ™zeĹ‚ lewym poddrzewem
            deleteWithOneLeftChild(current);
        } else if (current.left == null) { // jeĹ›li brak lewego potomka, zastÄ™pujemy wÄ™zeĹ‚ prawym poddrzewem
            deleteWithOneRightChild(current);
        } else {// wÄ™zeĹ‚ ma dwa potomki, zastÄ™pujemy go zatem jego nastÄ™pnikiem
    ...
    public void print() {//Przegladanie drzewa wraz z wypisywaniem elementow
        ...
        while (iterator.hasNext()) {// DopĂłki nie koniec elementow
